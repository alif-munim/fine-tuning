...
model_dirs = [d for d in os.listdir('.') if 'mistral-7b-instruct' in d and 'cluster' in d]
...
for group in set(re.match(r'mistral-7b-instruct-(\d+)_cluster', d).group(1) for d in model_dirs if re.match(r'mistral-7b-instruct-\d+_cluster', d)):
    ...
    if int(group) in select_groups:
        ...
        for model_dir in (d for d in model_dirs if d.startswith(f'mistral-7b-instruct-{group}_cluster')):
        ...
        avg_adapter_dir = f'mistral-7b-instruct-{group}_avg_adapter'
